# Understanding uncle-dev @spec Annotation Workflow

## Context

The user wants to understand how to use uncle-dev to analyze source code and `.md` spec files, identify and classify **LID feature labels**, and apply `@spec` annotations in documentation and source code.
...
## Workflow: Analyze Source + Spec Files to Identify Labels

1. **Read the source file** — identify public/exported functions that implement user-facing behaviors
2. **Check `docs/specs/`** — look for existing EARS IDs matching those behaviors
3. **If IDs exist** → annotate code entry points and matching tests
4. **If IDs are missing** → create them in `docs/specs/<segment>-specs.md` first, then annotate
5. **Run scanner** to verify: `python3 scan-spec-coherence.py --root "$(pwd)"`
6. **Regenerate graph** with `/uncle-dev-spec-graph` if the graph is in use
