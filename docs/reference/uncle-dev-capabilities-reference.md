...
`uncle-dev` is a Claude Code plugin (`version 1.4.1`, MIT) that packages production-grade engineering workflows for AI coding agents. It ships **22 slash commands**, **9 agent personas**, and **39 skills** covering the full software development lifecycle: **Define → Plan → Build → Verify → Review → Ship → Capture → Handoff → Maintain**.
