...
The installer copies the required files into the target project. No additional installation is required.

### 4. Open the target project in OpenCode

## Verify it worked

Confirm these files are present in your workspace:

- `AGENTS.md` (root)
- `skills/` directory
- `.opencode/agents/` directory (or `~/.config/opencode/agents/` for a global install)
...
This gives you an agent-driven workflow without plugins or manual commands.
